...
class Ray
  ...
  def hit_sphere(center, radius)
    # C = center, Q = ray.origin
    oc = center - origin
    # a = 2d
    a = direction.dot(direction)
    # b = -2d . * (C-Q)
    b = -2.0 * direction.dot(oc)
    # c = (C - Q) . (C - Q) - r * r
  ...
  def color(sphere_center, sphere_radius)
    hit = hit_sphere(sphere_center, sphere_radius)
    if hit > 0.0
      surface_normal = (at(hit) - sphere_center).unit_vector
      Color.new(surface_normal.x + 1, surface_normal.y + 1, surface_normal.z + 1) * 0.5
    else
...
def main(image_width: 1600)
  ...
  (0...image_height).each do |j|
    ...
    (0...image_width).each do |i|
      ...
      puts ray.color(sphere_center, sphere_radius)
    end
  end
end
